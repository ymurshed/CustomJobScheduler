...
using Quartzmin.Helpers.CronExpressionDescriptor;
...
namespace Quartzmin.Controllers
{
    ...
    {
        private readonly IProxyDbService _proxyDbService;

        public TriggersController(IProxyDbService proxyDbService)
        {
            _proxyDbService = proxyDbService;
        }
        ...
        public async Task<IActionResult> Save([FromForm] TriggerViewModel model)
        {
            ...
            if (result.Success)
            {
                ...
                {
                    await _proxyDbService.SaveTrigger(triggerModel);
                    await Scheduler.ScheduleJob(trigger);
                }
                else
                {
                    await _proxyDbService.SaveTrigger(triggerModel, true);
        ...
        public async Task<IActionResult> Delete([FromBody] KeyModel model)
        {
            ...
            await _proxyDbService.DeleteTrigger(key.Group.GetKey(key.Name));
            var hasDeleted = await Scheduler.UnscheduleJob(key);

            if (!hasDeleted) throw new InvalidOperationException("Cannot unschedule job " + key);
            return NoContent();
        }
        ...
        public IActionResult Cron()
        {
            ...
            try
            {
                desc = ExpressionDescriptor.GetDescription(cron);
            }
            catch
            { }
