...
namespace Quartzmin.Controllers
{
    public class JobsController : PageControllerBase
    {
        ...
        public JobsController(IProxyDbService proxyDbService)
        {
        ...
        public async Task<IActionResult> Save([FromForm] JobViewModel model, bool trigger)
        {
            ...
            if (result.Success)
            {
                ...
                if (jobModel.IsNew)
                {
                    await _proxyDbService.SaveJob(jobModel, jobDataMap);
                    await Scheduler.AddJob(BuildJob(JobBuilder.Create().StoreDurably()), true);
                }
                else
                {
                    await _proxyDbService.SaveJob(jobModel, jobDataMap, true);
        ...
        public async Task<IActionResult> Delete([FromBody] KeyModel model)
        {
            ...
            await _proxyDbService.DeleteJob(key.Group.GetKey(key.Name));
            var hasDeleted = await Scheduler.DeleteJob(key);
            
            if (!hasDeleted) throw new InvalidOperationException("Cannot delete job " + key);
            return NoContent();
        }
