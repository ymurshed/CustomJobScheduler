...
namespace Quartzmin
{
    ...
    {
        public static string GetJobDataAsJsonString(this JobDataMapItemBase[] itemBases)
        {
            var data = itemBases.ToDictionary(itemBase => itemBase.Name, itemBase => itemBase.Value);
            var jsonService = ServiceConfigurationInstance.ServiceProvider.GetService<IJsonService>();
            return jsonService.GetJsonStringFromObject(data);
        }

        public static string GetKey(this string group, string name)
        {
            return $"{group}.{name}";
        }
