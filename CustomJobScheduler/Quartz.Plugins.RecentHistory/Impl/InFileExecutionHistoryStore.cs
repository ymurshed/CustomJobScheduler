...
{
    [Serializable]
    public class InFileExecutionHistoryStore : IExecutionHistoryStore
    {
        ...
        public void LoadExecutionHistory()
        {
            lock (_data)
            {
                var executionHistoryStoreService = ServiceConfigurationInstance.ServiceProvider.GetService<IExecutionHistoryService>();
                _data = executionHistoryStoreService.GetExecutionHistory() != null ? 
                        (Dictionary<string, ExecutionHistoryEntry>) executionHistoryStoreService.GetExecutionHistory() :
                        new Dictionary<string, ExecutionHistoryEntry>();
            }
        }

        public void SaveExecutionHistory()
        {
            lock (_data)
            {
                var executionHistoryStoreService = ServiceConfigurationInstance.ServiceProvider.GetService<IExecutionHistoryService>();
                executionHistoryStoreService.SaveExecutionHistory(_data);
            }
        }
    }
}
